<picture>
  <source media="(prefers-color-scheme: dark) and (max-width: 600px)" srcset="assets/header-mobile-dark.svg">
  <source media="(max-width: 600px)" srcset="assets/header-mobile.svg">
  <source media="(prefers-color-scheme: dark)" srcset="assets/header-dark.svg">
  <img src="assets/header.svg" width="960" alt="A LITTLE RESEARCH NOTEBOOK — Hello, I'm Levi — Computational Social Science · Data Science · Applied AI — Columbia University · New York">
</picture>

I'm a **Columbia QMSS master's student** working at the intersection of **Computational Social Science, Data Science, and Applied AI**. My work focuses on reproducible data workflows, quantitative research, and practical research tools.

## Selected Work

<sub><code>PROJECT DIRECTORY / 04 FILES</code></sub>

<table>
  <tr>
    <td width="50%" valign="top">
      <img src="assets/project-iasc.svg" width="440" alt="DATA_01 — Donor Analytics">
      <strong><a href="https://github.com/leviycool/IASC-Prototype-Update">IASC Donor Analytics</a></strong>
      <p>A course prototype extension for querying synthetic donor data, with persistent conversations.</p>
      <sub>Python · Streamlit · SQLite · LLM APIs</sub>
      <p><a href="https://github.com/leviycool/IASC-Prototype-Update"><code>OPEN ↗</code></a></p>
    </td>
    <td width="50%" valign="top">
      <img src="assets/project-mail.svg" width="440" alt="MAIL_01 — Local Application">
      <strong><a href="https://github.com/leviycool/daily-email-digest">Daily Email Digest</a></strong>
      <p>IMAP retrieval and rule-based triage, with scheduled local digests and a SQLite cache.</p>
      <sub>Python · IMAP · SQLite · Tkinter</sub>
      <p><a href="https://github.com/leviycool/daily-email-digest"><code>OPEN ↗</code></a></p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <img src="assets/project-nlp.svg" width="440" alt="NLP_01 — Text Classification">
      <strong><a href="https://github.com/leviycool/stock-news-market-prediction">Stock News Market Prediction</a></strong>
      <p>Classical NLP models for daily news headlines, with Count/TF-IDF features, training-only feature fitting, and chronological evaluation.</p>
      <sub>Python · NLP · scikit-learn · Machine Learning</sub>
      <p><a href="https://github.com/leviycool/stock-news-market-prediction"><code>OPEN ↗</code></a></p>
    </td>
    <td width="50%" valign="top">
      <img src="assets/project-study.svg" width="440" alt="STUDY_01 — Study Tools">
      <strong><a href="https://github.com/leviycool/AI-Cheatsheet-assistant">AI Cheatsheet Assistant</a></strong>
      <p>Lecture documents to compact study sheets through extraction, concept prioritization, and export.</p>
      <sub>Python · Streamlit · OpenAI API</sub>
      <p><a href="https://github.com/leviycool/AI-Cheatsheet-assistant"><code>OPEN ↗</code></a></p>
    </td>
  </tr>
</table>

## Professional Contributions

<sub><code>WORK ARCHIVE</code> · World Bank · DIME</sub>

Contributions to public repositories maintained by World Bank DIME.

<table>
  <tr>
    <td width="50%" valign="top">
      <img src="assets/archive-boost.svg" width="440" alt="Work Archive — Public Finance">
      <strong><a href="https://github.com/dime-worldbank/mega-boost">MEGA-BOOST</a></strong>
      <p>Contributed to country-level ETL, validation, and harmonization for public finance data, including Burundi onboarding and Albania export fixes.</p>
      <sub>Python · Databricks · ETL · Data Validation</sub>
      <p><sub>Merged Contributions: <a href="https://github.com/dime-worldbank/mega-boost/pull/62">#62</a> · <a href="https://github.com/dime-worldbank/mega-boost/pull/60">#60</a></sub></p>
    </td>
    <td width="50%" valign="top">
      <img src="assets/archive-indicators.svg" width="440" alt="Work Archive — Population Data">
      <strong><a href="https://github.com/dime-worldbank/mega-indicators">MEGA-Indicators</a></strong>
      <p>Contributed to Burundi and Albania subnational population pipelines, including data quality fixes and region-name harmonization.</p>
      <sub>Python · Data Pipelines · Databricks · Data Quality</sub>
      <p><sub>Merged Contributions: <a href="https://github.com/dime-worldbank/mega-indicators/pull/39">#39</a> · <a href="https://github.com/dime-worldbank/mega-indicators/pull/31">#31</a> · <a href="https://github.com/dime-worldbank/mega-indicators/pull/30">#30</a></sub></p>
    </td>
  </tr>
</table>

### Research & Interests

Computational Social Science · Development Research · Applied ML & NLP · Causal Inference · Reproducible Data Workflows

### Toolkit

<picture>
  <source media="(max-width: 600px)" srcset="assets/toolkit-mobile.svg">
  <img src="assets/toolkit.svg" width="760" alt="System Tools: Python · R · SQL · Git · NLP · ML · Tableau">
</picture>

### Currently

M.A. in **Quantitative Methods in the Social Sciences**, Columbia University  
Expected December 2026 · New York

---

### Outside the Terminal

<sub><code>USER PROFILE</code></sub>

<img src="assets/pixel-decoration.svg" align="right" width="160" alt="Original pink pixel cat and silver game controller">

**Levi** · <code>STATUS: ONLINE</code>

<sub>PLAYING</sub><br>
Cyberpunk 2077 · Splatoon 3

<sub>SPORT</sub><br>
Table Tennis

<sub>CURRENT TASK</sub><br>
Building tiny tools for tiny problems.
